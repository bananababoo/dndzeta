...
for /f "tokens=*" %%i in ('curl -s https://fill.papermc.io/v2/projects/%PROJECT% ^| jq -r ".versions[-1]"') do set "LATEST_VERSION=%%i"
echo Latest Minecraft version is %LATEST_VERSION%

rem Get the latest build (including unstable) for the latest version
for /f "tokens=*" %%i in ('curl -s https://fill.papermc.io/v2/projects/%PROJECT%/versions/%LATEST_VERSION%/builds ^| jq -r ".builds[-1].build"') do set "LATEST_BUILD=%%i"
...
set "DOWNLOAD_URL=https://fill.papermc.io/v2/projects/%PROJECT%/versions/%LATEST_VERSION%/builds/%LATEST_BUILD%/downloads/%JAR_NAME%"
