...
object DatabaseActions {
    ...
    fun loadAll() {
        for (dataClass in Data::class.sealedSubclasses) {
            ...
            Data.Companion.dataLists[dataClass] =
    ...
    inline fun <reified T : Data> load() {
        ...
        Data.Companion.dataLists[T::class] = map
    }
    ...
    inline fun <reified T : Data> load(uuid: UUID): T? {
        ...
        Data.Companion.dataLists[T::class]?.put(uuid,data.first())
        return data[0]
    }

    inline fun <reified T : Data> save() {
        val saveData = (Data.Companion.dataLists[T::class]?.values as MutableSet<Data>).toTypedArray()
        if(saveData.isNotEmpty()) update(*saveData)
    }

    fun saveAll() {
        Data.Companion.dataLists.values.forEach {
    ...
    inline fun <reified T : Data> store(vararg objects: T) {
        ...
        val data = Data.Companion.dataLists[T::class]
    ...
    private inline fun <reified T : Data> add(obj: Array<out T>) {
        ...
        val data: MutableSet<Data> = Data.Companion.dataLists[T::class]?.values
            ?: throw IllegalStateException("Tried to get DataList that doesn't exist (shouldn't be possible) ${T::class.simpleName}")
        data.addAll(obj)
    }
